...
class RippleShader(rippleShape: RippleShape = RippleShape.CIRCLE) :
    ...
    companion object {
        ...
        const val RIPPLE_SPARKLE_STRENGTH: Float = 5.3f
        ...
        private const val SHADER_CIRCLE_MAIN =
            ...
            vec4 main(vec2 p) {
                ...
                // Create arc thickness
                float ring = smoothstep(radius - in_thickness, radius, dist) - 
                            smoothstep(radius, radius + in_thickness, dist);
                
                // Calculate angle for the arc
                float angle = atan(point.y, point.x);
                float normalizedAngle = (angle + 3.14159) / (2.0 * 3.14159);
                
                // Create progress-based arc
                float arcCutoff = smoothstep(normalizedAngle - 0.01, normalizedAngle, in_progress);
                
                // Calculate color based on battery level
                vec3 lowColor = vec3(1.0, 0.0, 0.0);  // Red
                vec3 highColor = vec3(0.0, 1.0, 0.0); // Green
                vec3 color = mix(lowColor, highColor, in_batteryLevel);
                
                // Combine all effects
                float alpha = ring * arcCutoff;
                return vec4(color, alpha);

                //float sparkle = sparkles(p - mod(p, in_pixelDensity * 0.8), in_time * 0.00175) * in_fadeSparkle;
                //float alpha = ring * arcCutoff;
                //vec3 finalColor = mix(color, vec3(1.0), sparkle * in_sparkle_strength);
                //return vec4(finalColor, alpha * in_color.a);
            }
            """
        ...
        private const val SHADER_CIRCLE_MAIN =
            ...
            vec4 main(vec2 p) {
                ...
                float sparkle = 2 * 10 + sparkles(p - mod(p, in_pixelDensity * 0.8), in_time * 0.00175)
    ...
    fun setBatteryLevel(level: Float) {
        setFloatUniform("in_batteryLevel", level)
    }

    fun setThickness(thickness: Float) {
        setFloatUniform("in_thickness", thickness)
    }
