 ...
 */
 ...
 open class RippleView(context: Context?, attrs: AttributeSet?) : View(context, attrs) {
     ...
     fun setupShader(rippleShape: RippleShape = RippleShape.CIRCLE) {
            ...
            // Se estiver usando o BORDER_SHADER, precisamos configurar o uniform "in_battery".
            // Aqui definimos um valor padrão (por exemplo, 1.0f para indicar bateria completa).
            if (rippleShape == RippleShape.BORDER) {
                
                rippleShader.setFloatUniform("in_battery", 0.0f) // MODIFICADO: seta nível de bateria para BORDER.
                rippleShader.setFloatUniform("in_thickness", 30.0f) 
            }
     ...
     /**
     * Novo método para atualizar o nível da bateria para o BORDER_SHADER.
     * O valor deve estar entre 0 (sem bateria) e 1 (bateria completa).
     */
    fun setBatteryLevel(battery: Float) {
        // MODIFICADO: Atualiza o uniform "in_battery" para animar a borda conforme o nível.
        rippleShader.setFloatUniform("in_battery", battery)
    }
