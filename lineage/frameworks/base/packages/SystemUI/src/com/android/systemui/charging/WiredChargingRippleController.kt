...
class WiredChargingRippleController @Inject constructor(
    ...
    @VisibleForTesting
    var rippleView: RippleView = RippleView(context, attrs = null).also { it.setupShader(RippleShape.BORDER) }
    ...
    fun registerCallbacks() {
        val batteryStateChangeCallback = object : BatteryController.BatteryStateChangeCallback {
            override fun onBatteryLevelChanged(
                ...
                // ADDED: Update the battery level on RippleView.
                // Convert the level (0-100) to a float between 0 and 1.
                rippleView.setBatteryLevel(level / 100f)
    ...
    private fun layoutRipple() {
        ...
        if (rippleView.rippleShape == RippleShape.BORDER) {
            // For the BORDER shader, use the actual screen dimensions.
            rippleView.setMaxSize(screenWidth, screenHeight)
        } else {
            val maxDiameter = Integer.max(screenWidth.toInt(), screenHeight.toInt()) * 2f
            rippleView.setMaxSize(maxDiameter, maxDiameter)
        }
        when (context.display?.rotation) {
            Surface.ROTATION_0 -> {
                rippleView.setCenter(screenWidth * normalizedPortPosX, screenHeight * normalizedPortPosY)
            }
            Surface.ROTATION_90 -> {
                rippleView.setCenter(screenWidth * normalizedPortPosY, screenHeight * (1 - normalizedPortPosX))
            }
            Surface.ROTATION_180 -> {
                rippleView.setCenter(screenWidth * (1 - normalizedPortPosX), screenHeight * (1 - normalizedPortPosY))
            }
            Surface.ROTATION_270 -> {
                rippleView.setCenter(screenWidth * (1 - normalizedPortPosY), screenHeight * normalizedPortPosX)
            }
        }
    }
