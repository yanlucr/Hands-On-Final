...
open class RippleView(context: Context?, attrs: AttributeSet?) : View(context, attrs) {
    ...
    private val textoBateria = Paint().apply {
    ...
    public var _batteryLevel: Float = 0f

    var duration: Long = 1750

    private val TAG = "SYSTEM_UI_ANIMATION"

    fun setMaxSize(maxWidth: Float, maxHeight: Float) {
        maxSize = (if (maxWidth < maxHeight) maxWidth else maxHeight).toFloat()
        rippleShader.rippleSize.setMaxSize(maxWidth, maxHeight)
    }

    fun updateBatteryLevel(level: Float) {
        _batteryLevel = level / 10f
        Log.d(TAG, "setBatteryLevel: level=$level, normalizado=$_batteryLevel")
        rippleShader.setBatteryLevel(_batteryLevel) // esse eh o tricky da cor variavel
        invalidate()
    }
    ...
    fun setupShader(rippleShape: RippleShape = RippleShape.CIRCLE) {
        ...
        rippleShader.pixelDensity = resources.displayMetrics.density
        rippleShader.setBatteryLevel(_batteryLevel)
    ...
    fun startRipple(onAnimationEnd: Runnable? = null) {
        ...
        animator.addUpdateListener { updateListener ->
            ...
            rippleShader.updateProgress(progress)
            rippleShader.setThickness(maxSize * 0.1f)
            rippleShader.setBatteryLevel(_batteryLevel)
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (rippleShape == RippleShape.CIRCLE) {
            Log.d(TAG, "THIS TAG... $_batteryLevel")

            val maskRadius = rippleShader.rippleSize.currentWidth
            canvas.drawCircle(centerX, centerY, maskRadius, ripplePaint)

            val text = "${_batteryLevel.toFloat()*10}%"
            textoBateria.textAlign = Paint.Align.CENTER


            val tamanhoFonteSp = 24f

            // Convertendo o valor de sp para pixels
            val tamanhoFontePx = TypedValue.applyDimension(
            TypedValue.COMPLEX_UNIT_SP,
            tamanhoFonteSp,
            resources.displayMetrics
            )

            // Aplicando o tamanho da fonte ao objeto Paint
            textoBateria.textSize = tamanhoFontePx

            // Calcula a posição vertical para centralizar o texto
            val yPos = centerY - (textoBateria.descent() + textoBateria.ascent()) / 2

            // Ajuste adicional para mover o texto para cima
            val verticalOffset = -300f // Valor negativo move o texto para cima
            canvas.drawText(text, centerX, yPos + verticalOffset, textoBateria)
